...
class AgentConfig(BaseModel):
    ...
    use_direct_llm_for_crow: bool = Field(
        default=False,
        description=(
            "If True, replace CROW agent calls with direct LLM calls. "
            "If False, use CROW platform agent."
        ),
    )
    use_direct_llm_for_falcon: bool = Field(
        default=False,
        description=(
            "If True, replace FALCON agent calls with direct LLM calls. "
            "If False, use FALCON platform agent."
        ),
    )
...
class RobinConfiguration(BaseModel):
    ...
    futurehouse_api_key: str = Field(
        default="insert_futurehouse_api_key_here",
        description="API key for FutureHouse platform (or set FUTUREHOUSE_API_KEY env var).",
    )
    service_uri: str = Field(
        default="https://dev.api.platform.edisonscientific.com",
        description="Base URI for the Edison platform API service.",
    )
    use_direct_llm_for_crow: bool | None = Field(
        default=None,
        description=(
            "If True, replace CROW agent calls with direct LLM calls. "
            "If False, use CROW platform agent. "
            "If None, uses value from agent_settings."
        ),
    )
    use_direct_llm_for_falcon: bool | None = Field(
        default=None,
        description=(
            "If True, replace FALCON agent calls with direct LLM calls. "
            "If False, use FALCON platform agent. "
    ...
    def set_run_folder_name_default(self) -> "RobinConfiguration":
        ...
        # Sync top-level flags to agent_settings if provided
        if self.use_direct_llm_for_crow is not None:
            self.agent_settings.use_direct_llm_for_crow = self.use_direct_llm_for_crow
        if self.use_direct_llm_for_falcon is not None:
            self.agent_settings.use_direct_llm_for_falcon = self.use_direct_llm_for_falcon
    ...
    def fh_client(self) -> EdisonClient:
        if self._fh_client is None:
            ...
            if not api_key or api_key == "insert_futurehouse_api_key_here":
                raise ValueError(
    ...
    def platform_base_url(self) -> str:
        """Returns the platform UI base URL derived from the service URI."""
        from .utils import get_platform_base_url
        return get_platform_base_url(self.service_uri)
